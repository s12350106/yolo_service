...
def draw_detections_on_image(image_data: bytes, detections: list):
    ...
    try:
        ...
        for detection in detections:
            ...
            # 繪製文字
            cv2.putText(img_resized, text, (x1, y1 - 5), 
                       font, font_scale, (0, 0, 0), thickness)
        
        # 編碼為JPEG
        success, buffer = cv2.imencode('.jpg', img_resized)
        if not success:
            raise Exception("無法編碼圖片為JPEG")
...
async def get_task_result_image(task_id: str):
    ...
    try:
        ...
        # 獲取原始圖片數據
        if hasattr(task, 'get_image_data'):
            image_data = task.get_image_data()
        elif hasattr(task, 'image_data'):
            image_data = task.image_data
        else:
            raise Exception("無法獲取原始圖片數據")
        
        # 在圖片上繪製檢測框
        image_with_boxes = draw_detections_on_image(image_data, detections)
